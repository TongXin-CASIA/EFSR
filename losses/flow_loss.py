...
class unFlowLoss(nn.modules.Module):
    def __init__(self, cfg):
        ...
        self.cfg = cfg
        if 'w_cnn' in self.cfg:
            self.loss_model = UTRNet('Architecture/AngleScale.pth')
        if 'w_perceptual' in self.cfg:
            self.perceptual_loss = PerceptualLoss(3)
    ...
    def loss_photomatric(self, im1_scaled, im1_recons, occu_mask1):
        ...
        if 'w_cnn' in self.cfg:
            self.loss_model.to(im1_recons.device)
            loss += [self.cfg.w_cnn * cnn_loss(im1_recons * occu_mask1,
                                               im1_scaled * occu_mask1, self.loss_model)]

        if 'w_perceptual' in self.cfg and self.cfg.w_perceptual > 0:
            self.perceptual_loss.to(im1_recons.device)
            loss += [self.cfg.w_perceptual * self.perceptual_loss(im1_recons * occu_mask1,
                                                                  im1_scaled * occu_mask1)]
...
@torch.no_grad()
def cnn_loss(im1, im2, model):
    im1 = torch.mean(im1, dim=1, keepdim=True)
    im2 = torch.mean(im2, dim=1, keepdim=True)
    feature1 = model(im1)
    feature2 = model(im2)
    # L1
    # loss = (feature1 - feature2).pow(2).sqrt().mean()
    # L2
    loss = (feature1 - feature2).pow(2).sum(1).sqrt().mean()
    return loss
...
class PerceptualLoss(nn.Module):
    ...
    def forward(self, x, y):
        ...
        loss = (x_vgg - y_vgg).abs().mean()
        return loss


class MaskedFlowLoss(unFlowLoss):
    def forward(self, output, target):
        """
        :param output: Multi-scale forward/backward flows n * [B x 4 x h x w]
        :param target: image pairs Nx6xHxW
